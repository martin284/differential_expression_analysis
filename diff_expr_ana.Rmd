...
I put the data into a specific format that is required for the DESeq analysis and performed the analysis. The result table shows the number of read counts for genes.
```{r, message=FALSE}
# fit format
...
The second quality check is done with PCA. The result shows differences among all the samples. PC1 is responsible for 97.7% of the variance and shows a large difference between the groups.
```{r}
...
I visualized the distribution of p-values before and after adjustment, which shows the effect of multiple testing corrections.
```{r}
...
In the plot above, there are many genes with high absolute log2 fold changes and low read counts. These data points are not reliable. That is why the log fold changes have been adjusted to the read counts. In the new plot, the number of outliers with low reliability could be significantly reduced. This result suggests the KD of FUBP1 does not lead to significant change of expression of U2-related genes.
```{r}
